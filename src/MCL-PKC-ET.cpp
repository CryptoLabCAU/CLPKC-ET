...
int main(int argc, char *argv[])
{
    double t0, t1;
    double TS = 0;
    double TEPV = 0, TEPK = 0, TEPPK = 0, TESK = 0, TVPK = 0;
    double TENC = 0, TDEC = 0;
    double TA = 0;
    double TT = 0;

    for (int itercnt = 0; itercnt < ITERCNT; itercnt++)
    {
        double S = 0;
        double EPV = 0, EPK = 0, EPPK = 0, ESK = 0, VPK = 0;
        double ENC = 0, DEC = 0;
        double A = 0;
        double T = 0;

        printf("MCL-PKC-ET : %d - Test \n", itercnt + 1);
        MCL_PKC_ET CLPKC(argc, argv);

        t0 = pbc_get_time();
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            CLPKC.init();
        }
        t1 = pbc_get_time();
        ...
        element_t msg;
        int lenG1 = CLPKC.getLenG1();
        unsigned char *IDi = new unsigned char[ID_SPACE];
        unsigned char *IDj = new unsigned char[ID_SPACE];
        unsigned char *MSG = new unsigned char[lenG1];

        memset(IDi, 0x00, ID_SPACE);
        memset(IDj, 0x00, ID_SPACE);
        memset(MSG, 0x00, lenG1);

        strcpy((char *)IDi, "IDIDIDIDIDID");
        strcpy((char *)IDj, "IDIDIDIDIDIDID");
        
        element_init_G1(msg, *CLPKC.getPairing());
        element_random(msg);
        element_to_bytes(MSG, msg);

        t0 = pbc_get_time();
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            unsigned char *xi = CLPKC.Extract_Private_Value();
            delete[] xi;
        }
        t1 = pbc_get_time();
        ...
        EPV = t1 - t0;
        TEPV += EPV;
        printf("Extract_Private_Value time = %f\n", EPV);

        unsigned char *xi = CLPKC.Extract_Private_Value();
        unsigned char *xj = CLPKC.Extract_Private_Value();

        // printf("Private-Value generation complete\n");


        t0 = pbc_get_time();
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            PK *pk = CLPKC.Extract_Public_Key(xi, IDi);
            delete [] pk->pk1;
            delete [] pk->pk2;
            delete [] pk->pk3;
            delete pk;
        ...
        EPK = t1 - t0;
        TEPK += EPK;
        printf("Extract_Public_Key time = %f\n", EPK);

        PK *pki = CLPKC.Extract_Public_Key(xi, IDi);
        PK *pkj = CLPKC.Extract_Public_Key(xj, IDj);
        // printf("Public-key generation complete\n");

        t0 = pbc_get_time();
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            KEY *dk = CLPKC.Extract_Partial_Private_Key(IDi);
            delete [] dk->key1;
            delete [] dk->key2;
            delete dk;
        }
        ...
        EPPK = t1 - t0;
        TEPPK += EPPK;
        printf("Extract_Partial_Private_Key time = %f\n", EPPK);

        KEY *dki = CLPKC.Extract_Partial_Private_Key(IDi);
        KEY *dkj = CLPKC.Extract_Partial_Private_Key(IDj);
        // printf("Partial-key generation complete\n");

        t0 = pbc_get_time();
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            KEY *sk = CLPKC.Extract_Private_Key(*dki, xi, *pki);
            delete [] sk->key1;
            delete [] sk->key2;
            delete sk;
        }
        ...
        ESK = t1 - t0;
        TESK += ESK;
        printf("Extract_Private_Key time = %f\n", ESK);

        KEY *ski = CLPKC.Extract_Private_Key(*dki, xi, *pki);
        KEY *skj = CLPKC.Extract_Private_Key(*dkj, xj, *pkj);
        // printf("Private-key generation complete\n");
        ...
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            CLPKC.Verify_Public_Key(*pki);
        }
        t1 = pbc_get_time();

        VPK = t1 - t0;
        TVPK += VPK;
        printf("Verification Public Key time = %f\n", VPK);
        ...
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            ...
            CIPHER *Ci = CLPKC.Encrypt(IDi, *pki, MSG);
            delete[] Ci->C1;
            delete[] Ci->C2;
            delete[] Ci->C3;
            delete[] Ci->C4;
        ...
        CIPHER *Ci = CLPKC.Encrypt(IDi, *pki, MSG);
        CIPHER *Cj = CLPKC.Encrypt(IDj, *pkj, MSG);
        // printf("Encryption complete\n");

        t0 = pbc_get_time();
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            unsigned char *MSG1 = CLPKC.Decrypt(*ski, *Ci, IDi, *pki);
            delete[] MSG1;
        }
        t1 = pbc_get_time();
        ...
        unsigned char *MSG1 = CLPKC.Decrypt(*ski, *Ci, IDi, *pki);
        unsigned char *MSG2 = CLPKC.Decrypt(*skj, *Cj, IDj, *pkj);
        ...
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            unsigned char *tdi = CLPKC.aut(*ski);
            delete[] tdi;
        }
        t1 = pbc_get_time();
        ...
        unsigned char *tdi = CLPKC.aut(*ski);
        unsigned char *tdj = CLPKC.aut(*skj);
        // printf("aut1 complete\n");

        t0 = pbc_get_time();
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            if (!CLPKC.test(*Ci, tdi, *Cj, tdj))
            {
                printf("test fails\n");
                abort();
            }
    ...
    printf("MCL-PKC-ET : %d X %d Test \n", ITERCNT, TESTCNT);
    printf("Setup total time = %f\n", TS);
    printf("Extract_Private_value total time = %f\n", TEPV);
    printf("Extract_Public_Key total time = %f\n", TEPK);
    printf("Extract_Partial_Private_Key total time = %f\n", TEPPK);
    printf("Extract_Private_Key total time = %f\n", TESK);
    printf("Verify_Public_Key total time = %f\n", TVPK);
    printf("Encryption total time = %f\n", TENC);
    printf("Decryption total time = %f\n", TDEC);
    printf("Authorization total time = %f\n", TA);
