...
int main(int argc, char *argv[])
{
    double t0, t1;
    double TS = 0;
    double TEPV = 0, TEPK = 0, TEPPK = 0, TESK = 0;
    ...
    {
        double S = 0;
        double EPV = 0, EPK = 0, EPPK = 0, ESK = 0;
        double ENC = 0, DEC = 0;
        double A = 0;
        double T = 0;

        printf("CL-PKC-ET : %d - Test \n", itercnt + 1);
        CL_PKC_ET CLPKC(argc, argv);
        ...
        EPV = t1 - t0;
        TEPV += EPV;
        printf("Extract_Private_Balue time = %f\n", EPV);
        ...
        for (int testcnt = 0; testcnt < TESTCNT; testcnt++)
        {
            KEY *pki = CLPKC.Extract_Public_Key(xi, IDi);
            delete[] pki->key1;
            delete[] pki->key2;
            delete pki;
        }
        ...
        KEY *pki = CLPKC.Extract_Public_Key(xi, IDi);
        KEY *pkj = CLPKC.Extract_Public_Key(xj, IDj);
