/**CL-PKC-ET.h**/
#ifndef __CL_PKC_ET_H__
#define __CL_PKC_ET_H__
...
#define Zp_SPACE 32 // n2
    ...
    public:
        CL_PKC_ET(/*int lambda = 80*/);
        CL_PKC_ET(int argc, char **argv);
        ~CL_PKC_ET();

        void init();
        void clear();
        
        CIPHER *Encrypt(const unsigned char *ID, const KEY pk, const unsigned char *M);
        unsigned char *Decrypt(const KEY sk, const CIPHER C, const unsigned char *ID, const KEY pk);

        unsigned char *Extract_Private_Value();
        KEY *Extract_Partial_Private_Key(const unsigned char *ID);
        KEY *Extract_Private_Key(const KEY dk, const unsigned char *x, const KEY pk);
        KEY *Extract_Public_Key(const unsigned char *x, unsigned char *ID);
